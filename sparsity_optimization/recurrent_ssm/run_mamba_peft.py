...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            if method == "sora_like":
                loss = loss + sora_lambda * mamba_l1_penalty(model)
...
def main():
    ...
    target_modules = ["in_proj", "state_proj", "out_proj"]
    ...
    metrics_path = os.path.join(args.output_dir, f"mamba_{args.method}_metrics.json")
    ckpt_path = os.path.join(args.output_dir, f"mamba_{args.method}_best.pt")
    ...
    print("=" * 80)
    print("FINAL SUMMARY")
    print(f"architecture      : mamba_like")
    print(f"method            : {args.method}")
